...
export default function ExplorePage() {
  ...
  const next = () => setCurrent(i => Math.min(carouselItems.length - 1, i + 1));
  ...
  const prevCat = () => setCatIndex(i => {
    const ni = Math.max(0, i - 1);
    scrollToCategory(ni);
    return ni;
  });
  const nextCat = () => setCatIndex(i => {
    ...
    <div className={styles.root}>
      ...
      <div className={styles.carouselWrapper}>
        ...
        <div className={styles.carouselArrows}>
          <button onClick={prev} className={styles.arrowButton} disabled={current === 0}>
            ‹
          </button>
          <button onClick={next} className={styles.arrowButton} disabled={current === carouselItems.length - 1}>
            ›
          </button>
        </div>
      ...
      <section className={styles.categorySection}>
        ...
        <div className={styles.categoryCarouselWrapper}>
          <button                      
            onClick={e => { e.preventDefault(); prevCat(); }}
            disabled={catIndex === 0}
          ...
          <button
           onClick={e => { e.preventDefault(); nextCat(); }}
      ...
      <div className={listStyles.listingWrapper}>
        ...
        <section className={listStyles.listingContent}>
          <div className={listStyles.listingHeader}>
            <div>Showing 1–{total} of {total} Results</div>
          </div>
                ...
                <div className={listStyles.countdown}>
                  {['Days','Hours','Min','Sec'].map(label => (
                    <div key={label} className={styles.timeSegment}>
                      <span>00</span><small>{label}</small>
                    </div>
                  ))}
                </div>
                <h3 className={listStyles.cardTitle}>{item.title}</h3>
                <div className={listStyles.currentBid}>
                  Current Bid at: <strong>${item.currentBid.toLocaleString()}</strong>
                </div>
                <button className={listStyles.bidButton}>
